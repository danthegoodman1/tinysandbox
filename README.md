...
| Area | Supported |
| --- | --- |
| `fs` (sync) | `readFileSync`, `readLinesSync` (tinysandbox UTF-8 line iterator), `writeFileSync`, `appendFileSync`, `mkdirSync`, `readdirSync` (incl. `withFileTypes`), `statSync`, `renameSync`, `rmSync`, `unlinkSync`, `rmdirSync`, `existsSync`, `copyFileSync`, `openSync`, `readSync`, `writeSync`, `ftruncateSync`, `closeSync` |
