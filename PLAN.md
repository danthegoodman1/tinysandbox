...
| In Progress | Test | local S3-compatible interoperability suite | Rust/live path complete: `tests/s3/compose.yaml`, `scripts/test-s3-compat.sh`, and `tests/vfs_s3.rs` pass against pinned MinIO with loopback-only endpoint guards, seeded adjacent-prefix secret, direct middle/tail/EOF ranges, VFS/Sandbox/shell/embedded-JS reads, read-only failures, and exact resource cleanup; separate Linux CI job added. Phase-1 fake tests remain authoritative for standards-shaped exact-object/descendant collisions because MinIO collapses that delimiter-list edge case. Missing: Node `s3Vfs` smoke coverage in Phase 3. |
